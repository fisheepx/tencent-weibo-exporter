...
class TencentWeibo:
    ...
    def getItems(self, stories):
        ...
        for story in stories:
            ...
            tencent_util.addAuthor(self.document, item[0])
            print '内容：', item[1]
            content_valid = ''.join(c for c in item[1] if ord(c) >= 32)# delete control chars
            tencent_util.addContent(self.document, content_valid)
            tencent_util.addPicture(self.document, story)
            print '时间：', item[2]
            tencent_util.addTime(self.document, item[2])
            tencent_util.addLocation(self.document, story)
